...
const part1 = async (): Promise<void> => {
    ...
    const fuelTotal = moduleMasses.map((mass: number): number => calculateFuel(mass)).reduce((previous: number, current: number): number => previous + current, 0);
...
const part2 = async (): Promise<void> => {
    ...
    const fuelTotal = moduleMasses.map((mass: number): number => calculateTotalFuel(mass)).reduce((previous: number, current: number): number => previous + current, 0);
    console.log("=== Part 2 ===");
